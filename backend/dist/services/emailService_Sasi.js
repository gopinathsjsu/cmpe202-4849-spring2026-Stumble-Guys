...
const email_Sasi_1 = require("../config/email_Sasi");
class EmailService {
    static async sendEmail(to, subject, html) {
        try {
            const resend = (0, email_Sasi_1.getResendClient)();
    ...
    static async sendTicketConfirmation(email, ticketDetails) {
        const html = `
    ...
    static async sendEventReminder(email, eventDetails) {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    static async sendApprovalNotification(email, eventTitle, status, notes) {
    ...
    static async sendRsvpDecisionEmail(email, input) {
    ...
    static async sendEventChangedEmail(email, input) {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>
          The event <strong>${input.eventTitle}</strong> was ${isCancelled ? 'cancelled' : 'updated'}${input.organizerName ? ` by ${input.organizerName}` : ''}.
        </p>
        <p style="color: #6b7280; font-size: 12px;">— The EventHub Team</p>
      </div>
    `;
    ...
    static async sendNewRsvpRequestEmail(email, input) {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #4f46e5;">New RSVP Request</h1>
        <p>${input.organizerName ? `Hi ${input.organizerName},` : 'Hi,'}</p>
        <p>
          You have a new RSVP request${input.attendeeName ? ` from <strong>${input.attendeeName}</strong>` : ''} for <strong>${input.eventTitle}</strong>.
        </p>
        <p style="color: #6b7280; font-size: 12px;">— The EventHub Team</p>
      </div>
    `;
    ...
    static async sendAdminNewEventPendingEmail(email, input) {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #4f46e5;">New Event Pending Approval</h1>
        <p>${input.adminName ? `Hi ${input.adminName},` : 'Hi,'}</p>
        <p>
          A new event <strong>${input.eventTitle}</strong>${input.organizerName ? ` by ${input.organizerName}` : ''} is pending approval.
        </p>
        <p style="color: #6b7280; font-size: 12px;">— The EventHub Team</p>
      </div>
    `;
