...
const generateSlug_Nikhil_1 = require("../utils/generateSlug_Nikhil");
const notificationService_Sasi_1 = require("./notificationService_Sasi");
const emailService_Sasi_1 = require("./emailService_Sasi");
const googleMapsUrl_Nikhil_1 = require("../utils/googleMapsUrl_Nikhil");
async function notifyAdminsEventPendingApproval(input) {
    const admins = await database_Preetam_1.default.user.findMany({
        where: { role: 'admin', is_active: true },
        select: { id: true, email: true, first_name: true },
    });
    await Promise.all(admins.map(async (admin) => {
        await notificationService_Sasi_1.NotificationService.createNotification(admin.id, 'event_pending_approval', 'New event pending approval', `"${input.eventTitle}" is awaiting review.`, input.eventId);
        if (admin.email) {
            emailService_Sasi_1.EmailService.sendAdminNewEventPendingEmail(admin.email, {
...
class EventService {
    static async createEvent(organizerId, input) {
        const slug = (0, generateSlug_Nikhil_1.generateSlug)(input.title);
        const parsedLatLng = input.google_maps_url && (!input.latitude || !input.longitude)
            ? (0, googleMapsUrl_Nikhil_1.parseGoogleMapsLatLng)(input.google_maps_url)
            : null;
        const created = await database_Preetam_1.default.$transaction(async (tx) => {
    ...
    static async listEvents(filters, page = 1, limit = 10) {
        ...
        const [events, total] = await Promise.all([
            ...
            database_Preetam_1.default.event.count({ where }),
        ]);
        return {
            data: events,
            pagination: {
    ...
    static async getEventBySlug(slug, requester) {
    ...
    static async updateEvent(id, organizerId, input) {
        const event = await database_Preetam_1.default.event.findUnique({ where: { id } });
        if (!event) {
            throw new Error('Event not found');
        }
        ...
        if (input.end_date !== undefined)
            data.end_date = new Date(input.end_date);
        ...
        if (input.google_maps_url !== undefined)
            data.google_maps_url = input.google_maps_url || null;
        ...
        if (input.google_maps_url &&
            ...
            const parsed = (0, googleMapsUrl_Nikhil_1.parseGoogleMapsLatLng)(input.google_maps_url);
            if (parsed) {
        ...
        if (input.online_url !== undefined)
            data.online_url = input.online_url;
        if (input.image_url !== undefined)
            data.image_url = input.image_url;
        if (input.capacity !== undefined)
            data.capacity = input.capacity;
        if (input.is_free !== undefined)
            data.is_free = input.is_free;
        ...
        const attendeeRows = await database_Preetam_1.default.user.findMany({
            where: {
                OR: [
        ...
        await Promise.all(attendeeRows.map(async (u) => {
            await notificationService_Sasi_1.NotificationService.createNotification(u.id, 'event_updated', 'Event updated', `"${updated.title}" has been updated.`, id);
            if (u.email) {
    ...
    static async deleteEvent(id, requesterId, requesterRole) {
        const event = await database_Preetam_1.default.event.findUnique({ where: { id } });
        if (!event) {
            throw new Error('Event not found');
        }
        ...
        const cancelled = await database_Preetam_1.default.event.update({
            where: { id },
        ...
        await Promise.all(attendeeRows.map(async (u) => {
            await notificationService_Sasi_1.NotificationService.createNotification(u.id, 'event_cancelled', 'Event cancelled', `"${cancelled.title}" was cancelled.`, id);
            if (u.email) {
                emailService_Sasi_1.EmailService.sendEventChangedEmail(u.email, {
                    recipientName: u.first_name,
    ...
    static async getEventsByOrganizer(organizerId, filters, page = 1, limit = 10) {
        const skip = (page - 1) * limit;
    ...
    static async submitForApproval(id, organizerId) {
        const event = await database_Preetam_1.default.event.findUnique({ where: { id } });
        if (!event) {
            throw new Error('Event not found');
        }
        ...
        const organizer = await database_Preetam_1.default.user.findUnique({
    ...
    static async getAttendees(eventId, organizerId) {
        ...
        return database_Preetam_1.default.ticket.findMany({
    ...
    static async getOrganizerDashboard(organizerId) {
        const now = new Date();
        const [totalActiveEvents, pendingAdminApproval, ticketAttendeeCount, rsvpApprovedCount, pendingRsvpRequests, upcomingEvents, notifPage,] = await Promise.all([
            database_Preetam_1.default.event.count({
                where: {
                    organizer_id: organizerId,
                    status: 'approved',
            ...
            notificationService_Sasi_1.NotificationService.getUserNotifications(organizerId, 1, 8),
        ]);
        return {
            metrics: {
                totalActiveEvents,
    ...
    static async getOrganizerPendingRsvps(organizerId) {
        const ownedEvents = await database_Preetam_1.default.event.findMany({
    ...
    static async getEventGuestlist(eventId, organizerId, search) {
        const event = await database_Preetam_1.default.event.findUnique({ where: { id: eventId } });
        if (!event) {
            throw new Error('Event not found');
        }
