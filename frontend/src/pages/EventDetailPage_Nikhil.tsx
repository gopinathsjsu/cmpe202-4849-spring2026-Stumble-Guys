              ...
              {event.google_maps_url ? (
                <div className="overflow-hidden rounded-xl border border-gray-200">
                  <iframe
                    ...
                    src={(() => {
                      // Best case: use lat/lng for a precise pin
                      if (event.latitude && event.longitude) {
                        return `https://maps.google.com/maps?q=${event.latitude},${event.longitude}&z=16&output=embed`;
                      }
                      const url = event.google_maps_url!;
                      // Already an embed URL
                      if (url.includes('google.com/maps/embed')) return url;
                      if (url.includes('output=embed')) return url;
                      // Extract lat,lng from any Google Maps URL
                      const latlngMatch = url.match(/@(-?\d+\.\d+),(-?\d+\.\d+)/);
                      if (latlngMatch) {
                        return `https://maps.google.com/maps?q=${latlngMatch[1]},${latlngMatch[2]}&z=16&output=embed`;
                      }
                      // Extract place name from place URL
                      ...
                      // Fallback: use venue name + full address for a precise pin
                      const address = [event.venue_name, event.address, event.city, event.state, event.zip_code]
                        .filter(Boolean).join(', ');
